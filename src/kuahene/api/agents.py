...
from kuahene.clients.wazuh import WazuhAPIClient
...
echo "=== Kuahene Agent Installer (Linux) ==="
...
if systemctl is-active --quiet wazuh-agent; then
    echo ""
    echo "SUCCESS: Kuahene agent is running and connecting to $MANAGER_IP"
    echo "Agent will appear in the Kuahene dashboard within 60 seconds."
else
    echo ""
...
echo "=== Kuahene Agent Installer (macOS) ==="
echo "Manager: $MANAGER_IP"
echo ""
...
WINDOWS_INSTALL = r"""# Kuahene Agent Installer — Windows
# Auto-generated for appliance: {manager_host}
...
Write-Host "=== Kuahene Agent Installer (Windows) ===" -ForegroundColor Cyan
...
if ($svc -and $svc.Status -eq "Running") {{
    Write-Host ""
    Write-Host "SUCCESS: Kuahene agent is running and connecting to $ManagerIP" -ForegroundColor Green
    Write-Host "Agent will appear in the Kuahene dashboard within 60 seconds."
}} else {{
    Write-Host ""
...
async def get_install_script(platform: str, request: Request) -> PlainTextResponse:
    ...
    if settings.appliance_host:
        ...
        request_host = request.headers.get("host", "").split(":")[0]
        manager_host = request_host if request_host and request_host not in ("localhost", "127.0.0.1", "0.0.0.0") else "<YOUR_KUAHENE_IP>"

    agent_group = "default"

    templates = {
        "linux": ("kuahene-install.sh", LINUX_INSTALL, "text/x-shellscript"),
        "macos": ("kuahene-install-macos.sh", MACOS_INSTALL, "text/x-shellscript"),
        "windows": ("kuahene-install.ps1", WINDOWS_INSTALL, "text/plain"),
    }
